...
object PresetUiFactory : Ui.Factory {
    override fun create(
        screen: Screen,
        context: CircuitContext
    ): Ui<*>? = when (screen) {
        is PresetScreen -> presetPage(screen.preset)
        else -> null
    }
}

fun presetPage(preset: Preset): Ui<State> = ui { state, modifier ->
    Scaffold(
        modifier = Modifier.padding(15.dp),
        floatingActionButton = {
            Button({ state.eventHandler(PresetScreen.Event.Close) }) {
                Row {
                    Text("Close")
                }
            }
            ...
            items(preset.demos, { it.description }) {
                Surface(Modifier.background(MaterialTheme.colorScheme.surfaceVariant)) {
                    Column(Modifier.fillMaxWidth()) {
                        ...
                        val result = remember { derivedStateOf { Structogram.fromString(it.code) } }
                        when (result.value) {
